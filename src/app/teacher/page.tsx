  ...
  async function onInviteStudent(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase
        .from("invited_emails")
        .upsert({ email: inviteEmail.trim().toLowerCase(), teacher_id: user.id }, { onConflict: "email" })
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      {loading ? (
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {toast && (
            <div className="rounded-md bg-blue-50 text-blue-800 border border-blue-200 px-3 py-2 text-sm w-fit">{toast}</div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* Room creation card */}
            <div className="rounded-xl border bg-white p-6">
              <h2 className="font-semibold mb-2">Create a Room</h2>
              <p className="text-sm text-gray-600 mb-4">Generate a room pass and share the QR with students.</p>
              ...
              {roomId && (
                <div className="mt-4 space-y-2">
                  ...
                  <p className="text-sm">Passcode: <span className="font-mono font-semibold">{passcode}</span></p>
                  {qr && (
                    <img alt="Room QR" src={qr} className="w-40 h-40 border rounded" />
                  )}
                  <div className="flex gap-2">
                    <button className="rounded-md border px-3 py-1 text-sm" onClick={() => router.push(`/room/${roomId}?as=teacher&name=${encodeURIComponent(name || "Teacher")}`)}>Go to Room</button>
                    <button className="rounded-md border px-3 py-1 text-sm" onClick={onEndSession}>End Session</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border bg-white p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">Add student email to whitelist. Students can sign up using this email.</p>

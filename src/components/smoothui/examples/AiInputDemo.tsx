...
import { SlashCommandDropdown, defaultCommands, SlashCommand } from "@/components/smoothui/ui/SlashCommandDropdown"
...
export default function AiInputDemo() {
  ...
  const [typedCommandName, setTypedCommandName] = React.useState<string | null>(null)
  ...
  const handleSlashCommand = (command: SlashCommand) => {
    ...
    if (slashIndex !== -1) {
      ...
      const newValue = beforeSlash.substring(0, slashIndex) + `/${command.name} ` + afterSlash
      setInputValue(newValue)
      setCursorPosition(slashIndex + command.name.length + 2)
      
      // Show parameter suggestions for quiz command
      if (command.name === 'quiz') {
        console.log('Quiz command selected! Available parameters:', command.parameters)
  ...
  const handleInputChange = (value: string, cursorPos: number) => {
    ...
    if (lastSlashIndex !== -1) {
      ...
      const commandMatches = defaultCommands.some(cmd => cmd.name.toLowerCase().startsWith(typedCommand.toLowerCase()))
      ...
      switch (e.key) {
        ...
        if (e.key === 'Enter' && inputValue.trim()) {
          ...
          if (lastSlashIndex !== -1) {
            ...
            const commandMatches = defaultCommands.some(cmd => cmd.name.toLowerCase() === typedCommand.toLowerCase())
